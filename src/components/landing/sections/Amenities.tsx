...
export default function AmenitiesSection({ amenities }: Props) {
  ...
  return (
    <section>
      <h2 className="text-xl font-semibold mb-4">Local Amenities</h2>
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {amenities.map(cat => (
          <div key={cat.category} className="rounded-lg border p-4 bg-background/50">
            <h3 className="font-medium mb-2 text-sm uppercase tracking-wide text-muted-foreground">{cat.category}</h3>
