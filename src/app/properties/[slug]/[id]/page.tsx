...
export default function PropertyDetailPage({ params }: { params: Promise<{ id: string }> }) {
  // keep your existing Promise unwrapping pattern
  const unwrappedParams = React.use(params)

  // fetch the main property detail as before
  const { data: propertyData, isLoading, isError } = usePropertyDetail(unwrappedParams.id)

  const faqs = propertyData?.faq_content ? JSON.parse(propertyData.faq_content) : []

  // ─────────────────────────────────────────────────────────────
  // NEW: DB-backed carousel images (from /api/properties/[listingKey]/media)
  // Prefers property_media rows; falls back to properties.media_urls
  // ─────────────────────────────────────────────────────────────
  const [dbImages, setDbImages] = React.useState<string[] | null>(null)

  React.useEffect(() => {
    let cancelled = false
    const id = unwrappedParams.id

    fetch(`/api/properties/${id}/media`, { cache: "no-store" })
      .then((r) => (r.ok ? r.json() : Promise.reject()))
      .then((d) => {
        if (cancelled) return
        const arr = Array.isArray(d?.images) ? d.images.map((x: any) => x?.url).filter(Boolean) : []
        setDbImages(arr)
      })
      .catch(() => { if (!cancelled) setDbImages([]) })

    return () => { cancelled = true }
  }, [unwrappedParams.id])

  // Normalize images from the legacy property payload (your original logic)
  const imagesNormalized = React.useMemo(() => {
    const imgs = (propertyData?.images || []).filter(Boolean)
    if (imgs.length > 0) return imgs
    if ((propertyData as any)?.main_photo_url) return [(propertyData as any).main_photo_url as string]
  ...
  // Prefer DB images if we have them; dedupe; then proxify
  const proxify = (url?: string | null) => {
    if (!url) return undefined
    if (url.startsWith("/")) return url // local assets untouched
    if (url.includes("/api/media?")) return url // already proxied
  ...
  const imagesFinal = React.useMemo(() => {
    const primary = dbImages && dbImages.length > 0 ? dbImages : imagesNormalized
    const seen = new Set<string>()
    return primary.filter((u) => {
      const key = (u || "").trim()
      if (!key || seen.has(key)) return false
      seen.add(key)
      return true
    })
  }, [dbImages, imagesNormalized])

  const imagesProxied = React.useMemo(() => imagesFinal.map((u) => proxify(u) || u), [imagesFinal])

  const propertyJsonLd = generatePropertyJsonLd(
    propertyData ? { ...propertyData, images: imagesFinal } as PropertyDetail : propertyData
  )
  ...
  return (
    <>
        ...
        <script
          dangerouslySetInnerHTML={{
            __html: `window.__activeProperty = ${JSON.stringify({
              ...
              hoa: (propertyData as any).hoa ?? (propertyData as any).hoa_monthly ?? undefined,
              home_insurance_annual: (propertyData as any).home_insurance_annual ?? undefined,
              image: (imagesFinal && imagesFinal[0]) || undefined,
            })};`,
          }}
        />
      ) : null}
      ...
      <div className="bg-neutral-50 dark:bg-slate-900 min-h-screen w-full pt-16 theme-transition">
        <section className="relative overflow-hidden">
          {/* ──────────────────────────────
              Carousel (now using DB images when available)
             ────────────────────────────── */}
          <Carousel className="w-full" data-carousel="main" opts={{ loop: true }}>
            <CarouselContent>
              {imagesProxied.map((src, index) => (
