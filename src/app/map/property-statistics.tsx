...
export default function PropertyStatistics({
  ...
  const stats = useMemo(() => {
    ...
    const avgPricePerSqft =
      pricePerSqft.reduce((sum, ppsf) => sum + ppsf, 0) / pricePerSqft.length;

    // Bedroom distribution
    const bedCounts = properties.reduce((acc, p) => {
      const beds = p.beds;
    ...
    if (propertiesWithYear.length > 0) {
      ...
      // Group by decade
      const decadeCounts = propertiesWithYear.reduce((acc, p) => {
    ...
    const propertiesWithFeatures = properties.filter(
      (p) => p.features && p.features.length > 0
    );

    if (propertiesWithFeatures.length > 0) {
      const featureCounts = propertiesWithFeatures.reduce((acc, p) => {
