...
import PropertiesGrid from "../../properties/properties-grid"
...
function HousesPageContent() {
  ...
  useEffect(() => {
    // Parse URL parameters but maintain the house category filter
    ...
    const updatedLegacyFilters = {
      ...
      sortBy: validSortBy,
      propertyCategory: "house" // Always filter by house
    }
    ...
    const newFilters: PropertyFilters = {
      ...filters,
      propertyCategory: ["house"], // Always maintain house filter
      ...
      beds: updatedLegacyFilters.minBedroom ? `${updatedLegacyFilters.minBedroom}+` : undefined,
      baths: updatedLegacyFilters.minBathroom ? `${updatedLegacyFilters.minBathroom}+` : undefined,
  ...
  const handleFilterChange = (newFilters: PropertyFilters) => {
    // Always maintain the house category filter
    ...
    if (updatedFilters.beds && updatedFilters.beds !== "Any") {
      params.set('minBedroom', updatedFilters.beds.replace('+', ''))
    }
    if (updatedFilters.baths && updatedFilters.baths !== "Any") {
      params.set('minBathroom', updatedFilters.baths.replace('+', ''))
    }
    if (updatedFilters.sortBy) params.set('sortBy', updatedFilters.sortBy)

    const newUrl = `/buy/houses${params.toString() ? '?' + params.toString() : ''}`
              ...
              {properties.length > 0 ? (
                ...
                <div className="text-center py-12">
                  <div className="text-gray-500 text-lg mb-4">
                    🏠 No houses found with current filters
                  </div>
